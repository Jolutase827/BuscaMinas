...
public class Menu {
    ...
    public static int selectLenght(){
        Scanner sc = new Scanner(System.in);
        String length;
        System.out.println("What size do you want the board to be (LARGE), (BIG), (ENORMOUS) or (EXTREMELY ENORMOUS)? ");
        length = sc.nextLine();
        while (boardLength(length)==-1){
            System.out.println("Please write (LARGE), (BIG), (ENORMOUS) or (EXTREMELY ENORMOUS): ");
            length = sc.nextLine();
        }
    ...
    public static int selectDificult(int boardLength){
        Scanner sc = new Scanner(System.in);
        String lvlDificult;
        System.out.println("What level of dificult do you want to be (EASY), (MEDIUM), (HARD) OR (IMPOSIBLE)?");
        lvlDificult = sc.nextLine();
        while (partyDificulty(lvlDificult,boardLength)==-1){
            System.out.println("Please write (EASY), (MEDIUM), (HARD) OR (IMPOSIBLE):");
            lvlDificult = sc.nextLine();
        }
    ...
    public static boolean activateHacks(){
        Scanner sc = new Scanner(System.in);
        String yesOrNo;
        System.out.println("Do you want to activate the hacks to see the bombs (yes or no)?");
        yesOrNo = sc.nextLine();
    ...
    public static String returnYesOrNo(String yesOrNo){
        ...
        while (yesOrNo.compareToIgnoreCase("yes")!=0&&yesOrNo.compareToIgnoreCase("no")!=0){
            System.out.println("Are you kidding me? Can you write (yes or no) for active the hacks pls?");
            yesOrNo = sc.nextLine();
        }
        return yesOrNo;
    }
